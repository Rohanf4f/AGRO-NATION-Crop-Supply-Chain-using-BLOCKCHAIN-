...
export default function WholesalerReceiveProduct(props) {
  ...
  async function handleSubmit() {
    ...
    if(verificationOutput) {
      ...
      supplyChain.methods.wholesalerReceivedcrop(address, manufacturer, signature).send({from: account})
  ...
  if (loading) { 
    return (
      <div>
        <p>Crop Package with address <b>{address}</b> received!</p>
      </div>
    );
  }
  ...
  return (
    <form className={classes.root} noValidate autoComplete="off">
      <TextField id="address" label="Crop Package Address" variant="outlined" onChange={ handleInputChange }/><br></br>
