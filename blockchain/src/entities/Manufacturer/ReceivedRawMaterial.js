...
export default function ReceivedRawMaterial(props) {
    ...
    async function getRawMaterialData() {
      ...
      let display = <div>
        <p>Generated Crop Product ID: {rawMaterialAddress}</p>
        <p>Crop Description: {data[1]}</p>
        <p>Crop Product Quantity: {data[2]}</p>
        <p>Crop Product Supplier: {data[3]}</p>
        <p>Crop Product Transporter: {data[4]}</p>
        <p>Crop Product Manufacturer: {data[5]}</p>
        <p>Crop Product Transaction contract address: <Link to={{ pathname: `/manufacturer/view-transaction/${data[6]}`, query: { address: data[6], account: account, web3: web3 } }}>{data[6]}</Link>
        </p>
        <CustomStepper
  ...
  function getSupplyChainStepContent(stepIndex) {
    switch (stepIndex) {
      case 0:
        return 'Crop Raw Material is at supplier stage in the supply chain.';
      case 1:
        return 'Crop Raw Material collected by the Transporter is on its way to the Manufacturer.';
      case 2:
        return 'Crop Raw Material currently with the Manufacturer';
      default:
  ...
  async function saveRawMaterialDetails() {
    ...
    if(loading) {
      ...
      return (
        <div>
          <h1>Crop Product Details</h1>
          <p>{details}</p>
          <Button variant="contained" color="primary" onClick={saveRawMaterialDetails}>Save Crop Raw Material Info to Database</Button>
        </div>
      );
    }
  } 